...
if (!empty($data)){
	...
	$rs_cliente = $conn_pedido -> prepare("SELECT * FROM cliente WHERE cod_cliente=".$cod_cliente."") or die ("Erro na String SELECT.");
	$rs_cliente -> execute();
	$row = $rs_cliente -> fetch(PDO::FETCH_OBJ);
	$empresa = $row -> empresa;
	...
	$str_carrinho = "SELECT p.descricao, c.preco_unidade, c.preco_total, c.quantidade, c.data, c.cod_carrinho FROM carrinho c, produto p WHERE c.cod_cliente=".$cod_cliente." AND c.cod_produto = p.cod_produto AND c.data='".$data."' ORDER BY p.descricao";
...
<?php if ($rs_produto -> execute()){
	 ?>
  	<form action="ope_pedido.php" name="form1" id="form1" method="post" >     
          ...
          <input type="hidden" value="<?php print $cod_cliente;?>" id="cod_cliente" name="cod_cliente">
        ...
        <button type="submit" class="btn btn-success btn-default" id="adicionar" name="adicionar">
        <span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Adicionar
        </button>
        
        <button type="button" onClick='javascript:location.href="pedido.php"' class="btn btn-default">
     ...
     <?php 
		...
		if (isset($_GET['msg_attention'])){
		 ...
		 if($rs_carrinho -> execute()){
             ...
             <?php print "<strong>Valor total do pedido: R$ ".$valor_total."</strong>";
			 		print "<br /><strong>Empresa: ".$empresa."</strong>"?>
         ...
         <table class="tablesorter">
                ...
                <tbody>
				 ...
				 while($row = $rs_carrinho -> fetch(PDO::FETCH_OBJ)){
							...
							?>
							<tr>
                            <td><a href="?msg_attention=<?php print $msg_attention;?>&data=<?php print $data;?>&cod_cliente=<?php print $cod_cliente;?>"><?php print $data;?></a></td>
							<td><a href="?msg_attention=<?php print $msg_attention;?>&data=<?php print $data;?>&cod_cliente=<?php print $cod_cliente;?>"><?php print $descricao;?></a></td>
							<td><a href="?msg_attention=<?php print $msg_attention;?>&data=<?php print $data;?>&cod_cliente=<?php print $cod_cliente;?>"><?php print $quantidade;?></a></td>
							<td><a href="?msg_attention=<?php print $msg_attention;?>&data=<?php print $data;?>&cod_cliente=<?php print $cod_cliente;?>"><?php print $preco_unidade;?></a></td>
							<td><a href="?msg_attention=<?php print $msg_attention;?>&data=<?php print $data;?>&cod_cliente=<?php print $cod_cliente;?>"><?php print $preco_total;?></a></td>
                      		<td align="center"><a href="ope_pedido.php?cod_carrinho=<?php print $cod_carrinho;?>&operacao=delete&data=<?php print $data;?>&cod_cliente=<?php print $cod_cliente;?>"><img title="Remover" src="../images/menu_adm/apagar.png"></a></td>
                            </tr>						 
				<?php 
				 } 
				 ?>      
        ...
        <button type="button" onClick='javascript:location.href="ope_pedido.php?operacao=finalizar&data=<?php print $data?>&cod_cliente=<?php print $cod_cliente;?>"' class="btn btn-success btn-default" id="finalizar" name="finalizar">
                ...
                <tbody>
				 ...
				 if($rs_produto -> execute()){
					 while($row = $rs_produto -> fetch(PDO::FETCH_OBJ)){
								...
								<tr><td><?php print $descricao;?></td>
								<td><?php print $preco;?></td>
								<td><?php print $tipo_venda;?></td></tr>
